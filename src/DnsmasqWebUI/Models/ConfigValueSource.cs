...
/// </remarks>
public record ConfigValueSource(string FilePath, string FileName, bool IsManaged)
{
    ...
    /// <summary>Returns a short tooltip for readonly values (which file and how to change). Null when <see cref="IsManaged"/> (editable).</summary>
    public string? GetReadOnlyTooltip() =>
        IsReadOnly ? $"From {FileName} (readonly). Edit {FilePath} to change." : null;
}
