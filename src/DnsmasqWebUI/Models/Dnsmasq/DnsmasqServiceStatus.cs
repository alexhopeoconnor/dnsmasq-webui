...
/// <param name="ManagedFilePath">Path to the app's managed config file (in ManagedFilesDirectory when configured, otherwise beside the main config).</param>
...
/// <param name="ConfigFiles">Ordered list of config files (main + conf-file + conf-dir) for "Config files (load order)" in the UI. Null or empty when config set is unavailable.</param>
...
public record DnsmasqServiceStatus(
    ...
    IReadOnlyList<DnsmasqConfigSetEntry>? ConfigFiles,
