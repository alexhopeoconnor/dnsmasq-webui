...
namespace DnsmasqWebUI.Infrastructure.Serialization.Parsers.DnsmasqConfig;
...
public static class DnsmasqConfIncludeParser
{
        ...
        HashSet<string>? seenPaths = null)
    {
        ...
        var lines = DnsmasqFileEncoding.ReadConfigLines(currentFilePath);
        ...
        for (var i = 0; i < lines.Length; i++)
        {
            ...
            if (string.Equals(key, DnsmasqConfKeys.ConfDir, KeyComparison))
            {
                ...
                resolvedDir = Path.GetFullPath(resolvedDir);
    ...
    public static string? GetFirstConfDir(string mainConfigPath)
    {
        ...
        foreach (var line in DnsmasqFileEncoding.ReadConfigLines(mainFull))
        {
            var kv = DnsmasqConfDirectiveParser.TryParseKeyValue(line);
            if (kv == null)
                continue;
